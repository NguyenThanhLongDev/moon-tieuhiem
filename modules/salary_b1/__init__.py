"""Module salary_b1 — MÔ PHỎNG lương theo phương án 2B (chưa phải bảng lương chính thức).
...
salary_b1_bp = Blueprint(
    "salary_b1", __name__,
    template_folder="templates",
    url_prefix="/salary-b1",
)

_CONFIG_KEY = "salary_b1_config"

# ── Cấu hình MẶC ĐỊNH — bản chạy thật chỉnh tại /salary-b1/config (lưu DB) ──
CONFIG = {
...
# Lương B1: CHỈ kế toán / IT / admin xem (siết hơn 2B — bỏ manager). Yêu cầu 19/6.
_FULL_VIEW_ROLES = {"admin", "superadmin", "ketoan", "accountant", "it"}
_CONFIG_EDIT_ROLES = {"admin", "superadmin"}


def get_config() -> dict:
    """Config 2B: bản lưu DB (app_config) đè lên mặc định. Sếp chỉnh tại /salary-b1/config."""
    ...
    try:
        ...
        logger.exception("salary_b1: lỗi đọc config DB — dùng mặc định")
    return cfg
...
def _require_full_view(f):
    ...
    def decorated(*args, **kwargs):
        if not session.get("user_id") and not session.get("username"):
            return redirect("/login?next=/salary-b1/")
...
def compute_salary_b1(year: int, month: int, cfg: dict | None = None):
    ...
    try:
        ...
        logger.exception("salary_b1: lỗi đọc hoàn thật — dùng hoàn ước")
...
def _cptest_key(year: int, month: int) -> str:
    return f"salary_b1_cptest_{year}_{month:02d}"
...
def get_cp_test(year: int, month: int) -> dict:
    ...
    try:
        ...
        logger.exception("salary_b1: lỗi đọc cp_test")
    return {}
...
def do_chin_don(year: int, month: int) -> dict:
    ...
    try:
        ...
        logger.exception("salary_b1: lỗi check độ chín đơn")
        return {"ket": 0, "tong": 0}
...
def index():
    try:
        ...
        year, month = 2026, 5
    cfg = get_config()
    rows, totals, warn_no_cong, (k_start, m_start) = compute_salary_b1(year, month, cfg)
...
def _adj_key(year: int, month: int) -> str:
    return f"salary_b1_adj_{year}_{month:02d}"
...
def get_adjustments(year: int, month: int) -> dict:
    ...
    try:
        ...
        logger.exception("salary_b1: lỗi đọc adjustments")
    return {}


@salary_b1_bp.route("/adj", methods=["POST"])
@_require_full_view
def save_adjustments():
    ...
    return redirect(request.referrer or "/salary-b1/")


@salary_b1_bp.route("/huong-dan")
@_require_full_view
def huong_dan():
    ...
    return render_template("salary_b1/huong_dan.html", md_content=md)


@salary_b1_bp.route("/cp-test", methods=["POST"])
@_require_full_view
def save_cp_test():
...
@salary_b1_bp.route("/config", methods=["GET", "POST"])
@_require_full_view
def config_page():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect("/salary-b1/config")
    ...
    return render_template("salary_b1/config.html", config=get_config(),
                           can_edit=can_edit, users=users)


def register_salary_b1_module(app):
    app.register_blueprint(salary_b1_bp)
    logger.info("salary_b1 module registered at /salary-b1/")
